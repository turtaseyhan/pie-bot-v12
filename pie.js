...
client.on("ready", async () => {
  ...
  client.user.setUsername(process.env.clientname)
});
        ...
        new Discord.MessageEmbed()
          .setThumbnail(
            mention.user.avatarURL()
              ? mention.user.avatarURL({ dynamic: true })
              : process.env.clientAvatar
          )
          .setTitle("Tıkladığın Kullanıcı AFK")
          .setDescription(` \n**• __Sebep;__ \`${est}\`**`)
      );
    }
  }
  const stat = await data.fetch(
  ...
  if (stat) {
    ...
    message.channel.send(
      new Discord.MessageEmbed().setDescription(
        `${message.author} **Cihaz üzerine tekrardan hoş geldin!**`
      )
    );
  }
});
...
client.on("guildBanAdd", async member => {
  const sistem = await data.fetch(`korumalar.${member.guild.cache.get.id}`);
  if (!sistem) return;
...
client.on("message", message => {
  ...
  if (mesaj.includes("Pie")) message.react("🤫");
});
...
client.on("message", async message => {
  ...
  if (message.content.toLocaleLowerCase() === "sa") {
    ...
    return message.channel.send(
      new Discord.MessageEmbed().setDescription(
        `${message.author} **ve aleyküm selam, Hoş Geldin!**`
      )
    );
  }
});
...
client.on("guildMemberAdd", async member => {
  ...
  member.send(
    new Discord.MessageEmbed()
      ...
      .addField(
        "• Bilgilendirme",
        "**Sunucu içerisinde ki yetkililere ulaşarak yasaklı tag dan cıkmanızı sağlayabilirsiniz!"
      )
  );
});
...
client.on("guildMemberAdd", async member => {
  ...
  let systemChannel;
  if (systemChannelData)
    systemChannel = guild.channels.cache.get(systemChannelData);
  ...
  if (systemChannel)
    systemChannel.send(
      `${member} giriş yaptı. Değişiklik: ${user.username} -> ${replacedName}`
    );
});
  ...
  if (user.createdAt.getTime() <= 432000000) {
    ...
    member.user.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `>>> \`${guild.name}\` __Sunucusu için, Yeni hesap olduğunuzu tespit ettim. **5 Gün** içerisinde olan hesapları cezalıya atıyorum!__`
        )
        .addField(
          "• Bilgilendirme",
          "**Sunucu içerisinde ki yetkililere bildirmelisiniz.**"
        )
        .setColor("#351742")
    );
  }
