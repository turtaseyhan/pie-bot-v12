...
exports.run = async (client, message, args) => {
  if (message.author.id === message.guild.owner.user.id)
    return message.channel.send(
      new Discord.MessageEmbed().setTitle(
        "**Seni gidi seni, Sunucu sahibisin sana __AFK__ yakışmaz ^^**"
      )
    );
  ...
  message.channel
    .send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `${message.author} **Üyesi Bilgisayar başında & Klavye başında değil**!`
        )
    )
  ...
  data.set(`kullanıcı.${message.author.id}.${message.guild.id}`);
};
exports.conf = {
  enabled: true,
  guildOnly: true,
