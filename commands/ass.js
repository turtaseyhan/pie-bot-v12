...
if(await data.fetch(`slm.${message.author.id}.${message.guild.id}.ass`)) return message.channel.send(new Discord.MessageEmbed().setColor('#00001').setDescription(`**5 saniyenin altında hızlı kullanamazsın!**`));
DabiClient.nsfw.real.ass().then(json => {
message.channel.send(new Discord.MessageAttachment(json.url));
data.set(`slm.${message.author.id}.${message.guild.id}.ass`, 'Code World');
setTimeout(() => {
